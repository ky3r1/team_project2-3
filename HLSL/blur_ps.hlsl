...
float4 main(VS_OUT pin) : SV_TARGET
{
    ...
    for (int x = -gaussian_half_kernel_size; x<=gaussian_half_kernel_size; x+=1)
    {
        ...
        {
            float gaussian_kernel = exp(-(x * x + y* y)/(2.0f* gausian_sigma * gausian_sigma)/
                (2 * 3.14159265358979 * gausian_sigma * gausian_sigma));
            blue_color += texture_maps[1].Sample(sampler_states[LINEAR], pin.texcoord +
                float2(x * 1.0 / width, y * 1.0 / height)).rgb * gaussian_kernel;
            gaussian_kernel_total += gaussian_kernel;
        }
    }
    blue_color /= gaussian_kernel_total;
    //bloom_intensity = 1.0f;
    color.rgb += blue_color * bloom_intensity;
